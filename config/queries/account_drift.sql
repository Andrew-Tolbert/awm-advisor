...
SELECT
  ...
  CASE
    ...
    ELSE account_name
  END AS account_name,
  ROUND(total_account_value / 1e6, 4)                              AS account_aum,
  asset_class,
  ROUND(actual_market_value / 1e6, 4)                              AS actual_dollars,
  ...
  ROUND(target_market_value / 1e6, 4)                              AS target_dollars,
  ROUND(min_market_value / 1e6, 4)                                 AS min_dollars,
  ROUND(max_market_value / 1e6, 4)                                 AS max_dollars,
  ROUND(drift_from_target_pct, 2)                                  AS delta_pct,
  ROUND((actual_market_value - target_market_value) / 1e6, 4)      AS delta_dollars,
  drift_status,
  ROUND(ABS(band_distance_pct), 2)                                 AS band_distance_pct,
  ROUND(rebalance_to_band / 1e6, 4)                                AS rebalance_to_band,
  ROUND(rebalance_to_target / 1e6, 4)                              AS rebalance_to_target,
  risk_profile,
  drift_severity
